...
onMounted(() => {
    if (navigator.clipboard) {
        setInterval(async () => {
            const hasPermission = await navigator.permissions.query({ name: 'clipboard-read' });

            if (hasPermission.state !== 'granted' || !document.hasFocus() || !settings.pasteFromClipboard.value) {
                return
            }
            ...
            <div class="space-y-3">
                <label for="pasteFromClipboard" class="flex items-center">
                    <input
                        ...
                        v-model="settings.pasteFromClipboard.value"
                        type="checkbox"
                        class="mr-2"
                    >
                ...
                <label for="saveToHistory" class="flex items-center">
                    <input
                        id="saveToHistory"
                        v-model="settings.saveToHistory.value"
                        type="checkbox"
                        class="mr-2"
                    >
